 ...
 * look like in NanoGUI.  Run the one of the :ref:`waylandgui_example_icons`
 * executables to see what they all look like in NanoGUI.
 *
 * .. tip::
 *
 *    In C++, ``#include <waylandgui/icons.h>`` to gain access to the ``#define``
 *    shown in these docs and simply reference the names as listed in the
 *    documentation.  To use the icons from Python, declare
 *    ``from waylandgui import icons`` and reference the icons as follows:
 *    ``icons.FA_ASTERISK``.
 *
