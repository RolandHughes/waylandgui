...
#define WAYLANDGUI_VERSION_MAJOR 0
#define WAYLANDGUI_VERSION_MINOR 1
#define WAYLANDGUI_VERSION_PATCH 4

#define WAYLANDGUI_STRINGIFY(x) #x
#define WAYLANDGUI_TOSTRING(x)  WAYLANDGUI_STRINGIFY(x)
#define WAYLANDGUI_VERSION                                                          \
    (WAYLANDGUI_TOSTRING(WAYLANDGUI_VERSION_MAJOR) "."                                 \
     WAYLANDGUI_TOSTRING(WAYLANDGUI_VERSION_MINOR) "."                                 \
     WAYLANDGUI_TOSTRING(WAYLANDGUI_VERSION_PATCH))

/* Set to 1 to draw boxes around widgets */
//#define WAYLANDGUI_SHOW_WIDGET_BOUNDS 1

#if !defined(NAMESPACE_BEGIN) || defined(DOXYGEN_DOCUMENTATION_BUILD)
    /**
     * \brief Convenience macro for namespace declarations
     *
     * The macro ``NAMESPACE_BEGIN(waylandgui)`` will expand to ``namespace
     * waylandgui {``. This is done to hide the namespace scope from editors and
     ...
     * ``NAMESPACE_END(waylandgui)`` will expand to only ``}``.
     *
     * \param name
...
#if defined(WAYLANDGUI_SHARED)
#  if defined(WAYLANDGUI_BUILD)
#    define WAYLANDGUI_EXPORT __attribute__ ((visibility("default")))
#  else
#    define WAYLANDGUI_EXPORT
#  endif
#else
     /**
      * If the build flag ``WAYLANDGUI_SHARED`` is defined, this directive will expand
...
#define WAYLANDGUI_FORCE_DISCRETE_GPU()
 ...
 *
 * \rst
 * Refer to :ref:`waylandgui_example_3` for how you might go about managing OpenGL
 * and GLFW on your own, while still using NanoGUI's classes.
 * \endrst
 */
extern WAYLANDGUI_EXPORT void init();

/// Static shutdown; should be called before the application terminates.
extern WAYLANDGUI_EXPORT void shutdown();
 ...
 */
extern WAYLANDGUI_EXPORT void async(const std::function<void()> &func);
 ...
 */
extern WAYLANDGUI_EXPORT std::string
 ...
 */
extern WAYLANDGUI_EXPORT std::vector<std::string>
file_dialog(const std::vector<std::pair<std::string, std::string>> &filetypes,
 ...
 */
extern WAYLANDGUI_EXPORT void chdir_to_bundle_parent();
#endif
 ...
 * defined in :ref:`file_waylandgui_entypo.h`.
 * \endrst
 *
 * \param c
 *     The UTF32 character to be converted.
 */
extern WAYLANDGUI_EXPORT std::string utf8(uint32_t c);

/// Load a directory of PNG images and upload them to the GPU (suitable for use with ImagePanel)
extern WAYLANDGUI_EXPORT std::vector<std::pair<int, std::string>>
...
#define nvgImageIcon(ctx, name) waylandgui::__waylandgui_get_image(ctx, #name, name##_png, name##_png_size)
/// Helper function used by nvg_image_icon
extern WAYLANDGUI_EXPORT int __waylandgui_get_image(NVGcontext *ctx, const std::string &name,
