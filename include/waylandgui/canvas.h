...
/**
 * \class GLCanvas canvas.h waylandgui/canvas.h
 *
 * \brief Canvas widget for rendering Wayland content
 *
 * Canvas widget that can be used to display arbitrary GLFW3 Wayland content. 
 * This is useful to display and manipulate 3D objects as part of an interactive
 * application. The implementation uses scissoring to ensure that rendered
 * objects don't spill into neighboring widgets.
 *
 * \rst
 * **Usage**
 *     Override :func:`waylandgui::GLCanvas::draw_contents` in subclasses to provide
 *     custom drawing code.  See :ref:`waylandgui_example_4`.
 *
 * \endrst
 */
class WAYLANDGUI_EXPORT Canvas : public Widget {
public:
    /**
