...
/**
 * \file waylandgui/shader.h
 *
 * \brief Defines abstractions for shaders that work with 
 * GLFW3 Wayland
 */
...
#include <waylandgui/traits.h>
#include <unordered_map>

NAMESPACE_BEGIN(waylandgui)

class WAYLANDGUI_EXPORT Shader : public Object {
public:
    ...
    template <typename Array> void set_uniform(const std::string &name,
        ...
        } else if constexpr (is_waylandgui_array_v<Array>) {
            data = value.v;
            ndim = 1;
            shape[0] = Array::Size;
            vtype = get_type<typename Array::Value>();
        } else if constexpr (is_waylandgui_matrix_v<Array>) {
            data = value.m;
            ndim = 2;
            shape[0] = Array::Size;
            shape[1] = Array::Size;
...
/// Access binary data stored in waylandgui_resources.cpp
#define WAYLANDGUI_RESOURCE_STRING(name) std::string(name, name + name##_size)

/// Access a shader stored in waylandgui_resources.cpp
#  define WAYLANDGUI_SHADER(name) WAYLANDGUI_RESOURCE_STRING(name##_gles)
