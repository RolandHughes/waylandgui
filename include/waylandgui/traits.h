...
extern WAYLANDGUI_EXPORT size_t type_size(VariableType type);

/// Return the name (e.g. "uint8") associated with a specific variable type
extern WAYLANDGUI_EXPORT const char *type_name(VariableType type);
...
namespace detail {
    ...
    template <typename T> using is_waylandgui_array_det  = std::enable_if_t<T::IsNanoGUI && !T::IsMatrix>;
    template <typename T> using is_waylandgui_matrix_det = std::enable_if_t<T::IsNanoGUI && T::IsMatrix>;
}
...
template <typename T>
constexpr bool is_waylandgui_array_v = is_detected_v<detail::is_waylandgui_array_det, std::decay_t<T>>;

template <typename T>
constexpr bool is_waylandgui_matrix_v = is_detected_v<detail::is_waylandgui_matrix_det, std::decay_t<T>>;
